...
/**
 * NationalStrategy — broadest fallback: same make+model, any year
 */
export class NationalStrategy implements IPricingStrategy {
  readonly name = PricingStrategy.NATIONAL;
  ...
  async compute(input: PricingInput, pool: Pool): Promise<MarketStats> {
    ...
    const r = rows[0];
    return {
      strategyUsed: PricingStrategy.NATIONAL,
      ...
      filters:    { make: vehicle.make, model: vehicle.model },
    };
  }
}
